...
Starter.prototype.prepare = function(main,message,clients){
    ...
    for(var ipindex in clients) {
        for (var i = 0;i<count;i++) {
            ...
            var cmd = 'cd ' + cwd + ' && ' + execPath + ' ' + fixMain + ' client > log/agent-' + i + '.log';
...
Starter.prototype.sshrun = function (cmd,host,callback) {
	  var hosts = [host];
    console.log('Executing ' + $(cmd).yellow + ' on ' + $(hosts.join(', ')).blue);
    var wait = 0;
    data = [];
    if (hosts.length > 1) {
...
Starter.prototype.localrun = function (cmd, callback) {
    console.log('Executing ' + $(cmd).green + ' locally');
...
Starter.prototype.load = function (file) {
    ...
    console.log('Executing compile ' + file);
